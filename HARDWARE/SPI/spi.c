...
void SPI1_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(LCD_SCL_CLK, ENABLE);//GPIOA
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_SPI1, ENABLE);//使能SPI1  APB2 84Mhz
	//SCL
  ...
  GPIO_Init(LCD_SCL_Port, &GPIO_InitStructure);//初始化
	...
	GPIO_PinAFConfig(LCD_SCL_Port, LCD_SCLK_Source, GPIO_AF_SPI1);//复用配置
	GPIO_PinAFConfig(LCD_SCL_Port, LCD_SDA_Source, GPIO_AF_SPI1);
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;//波特率预分频值：波特率预分频值为2	42mhz
...
void SPI2_Init(void)
{
  ...
  GPIO_Init(LCD_Rem_Port, &GPIO_InitStructure);//初始化
	...
	SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;//串行同步时钟空第二个时钟沿捕获
	SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;//NSS信号由软件管理
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;//波特率预分频值：波特率预分频值为2	24mhz
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;//数据传输高位先行
	SPI_InitStructure.SPI_CRCPolynomial = 7;//CRC值计算的多项式
